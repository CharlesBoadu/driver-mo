...
function DashboardPage() {
  ...
  useEffect(() => {
    ...
    const onboardingData = JSON.parse(
      localStorage.getItem("driverMoOnboarding")
    );
  ...
  return (
    <>
      <Helmet>
        ...
        <meta
          name="description"
          content="Manage your Driver Mo insurance policies, view purchase history, file claims, and more."
        />
      </Helmet>
      <DashboardLayout user={user}>
        <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
          <TabsList className="grid w-full grid-cols-2 md:grid-cols-6 bg-orange-100/80 rounded-lg p-1 h-auto mb-6">
            {tabs.map((tab) => (
              <TabsTrigger
                key={tab.value}
                value={tab.value}
                className="flex-col sm:flex-row h-14 sm:h-12 data-[state=active]:bg-orange-500 data-[state=active]:text-white data-[state=active]:shadow-md"
              >
